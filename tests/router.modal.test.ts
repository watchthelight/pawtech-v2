...
import { BTN_DECIDE_RE, BTN_VIEW_SRC_RE, identifyModalRoute } from "../src/lib/modalPatterns.js";
...
describe("button patterns", () => {
  ...
  it("matches avatar view source buttons", () => {
    const match = BTN_VIEW_SRC_RE.exec("v1:avatar:viewsrc:codeBADF00");
    expect(match).not.toBeNull();
    expect(match?.[1]).toBe("BADF00");
  });
});
