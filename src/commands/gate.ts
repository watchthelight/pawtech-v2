...
export async function executeAccept(ctx: CommandContext<ChatInputCommandInteraction>) {
  ...
  if (cfg) {
    const flow = await approveFlow(interaction.guild, app.user_id, cfg);
  ...
  try {
    await closeModmailForApplication(interaction.guildId, app.user_id, code, {
      reason: "approved",
      client: interaction.client,
      guild: interaction.guild,
    });
  } catch (mmErr) {
    logger.warn({ err: mmErr, appId: app.id }, "[accept] Failed to close modmail (non-fatal)");
  }
  ...
  try {
    ...
    logger.warn({ err, appId: app.id }, "Failed to refresh review card after /accept");
  }
...
export async function executeReject(ctx: CommandContext<ChatInputCommandInteraction>) {
  ...
  ctx.step("reject_tx");
  const tx = rejectTx(app.id, interaction.user.id, reason, permanent);
  if (tx.kind === "already") {
  ...
  try {
    ...
    logger.warn({ err: mmErr, appId: app.id }, "[reject] Failed to close modmail (non-fatal)");
  }
  ...
  try {
    ...
    logger.warn({ err, appId: app.id }, "Failed to refresh review card after /reject");
  }
