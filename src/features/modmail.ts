...
async function flushTranscript(params: {
  ...
  const lines = transcriptBuffers.get(ticketId);
  ...
  try {
    ...
    // If no transcript lines, just post "No content" log
...
export function insertModmailMessage(map: ModmailMessageMap) {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (@ticketId, @direction, @threadMessageId, @dmMessageId, @replyToThreadMessageId, @replyToDmMessageId)
...
export async function routeThreadToDm(message: Message, ticket: ModmailTicket, client: Client) {
  ...
  try {
    ...
    // Append to transcript buffer for audit trail
