...
const VIEW_SRC_BUTTON_RE = BTN_VIEW_SRC_RE;
const VIEW_SRC_MODAL_RE = MODAL_18_RE;
...
export function findAppByShortCode(guildId: string, code: string): ApplicationRow | null {
  ...
  for (const row of rows) {
    if (shortCode(row.id) === normalized) {
      return row;
    }
  }
  return null;
...
async function handleClaimToggle(interaction: ButtonInteraction, app: ApplicationRow) {
  // Reload fresh application state to check current status
  const freshApp = db.prepare(`SELECT status FROM application WHERE id = ?`).get(app.id) as
    | { status: ApplicationStatus }
    | undefined;
  ...
  if (!freshApp) {
    ...
    logger.warn({ appId: app.id }, "[review] Claim attempt on missing application");
    return;
  }

  // Check if application is in a claimable state
  if (!isClaimable(freshApp.status)) {
    await replyOrEdit(interaction, {
      content: `Cannot claim: application is already **${freshApp.status}**.`,
      flags: MessageFlags.Ephemeral,
    }).catch(() => undefined);
    logger.info(
      { appId: app.id, status: freshApp.status, moderatorId: interaction.user.id },
      "[review] Claim attempt blocked - application not claimable"
    );

    // Refresh card to show current state
    try {
      await ensureReviewMessage(interaction.client, app.id);
      logger.info({ appId: app.id }, "[review] card refreshed after blocked claim");
    } catch (err) {
      logger.warn({ err, appId: app.id }, "[review] failed to refresh card after blocked claim");
    }
    return;
  }
  ...
  const existing = getClaim(app.id);

  // Check if someone else claimed it
  const claimError = claimGuard(existing, interaction.user.id);
  if (claimError) {
    await replyOrEdit(interaction, {
      content: claimError,
      flags: MessageFlags.Ephemeral,
  ...
  // If this user already claimed it, treat it as idempotent (refresh the card)
  if (existing && existing.reviewer_id === interaction.user.id) {
    logger.info(
      { appId: app.id, claimerId: interaction.user.id },
      "[review] user re-claimed (idempotent)"
    );
    // Continue to refresh the card
  }

  // Note: deferUpdate() already called by parent handleReviewButton, so don't call again

  // Claim the application in DB with review_action insert (atomic transaction)
  const claimActionId = db.transaction(() => {
    // Upsert claim
    upsertClaim(app.id, interaction.user.id);

    // Insert review_action for audit trail
    const result = db
      .prepare(
        `
...
export async function handleCopyUidButton(interaction: ButtonInteraction) {
  ...
  try {
    ...
    // Reply with UID in copyable format (triple backticks give Discord's native Copy button)
    await interaction.reply({
      content: `**Applicant UID:**\n\`\`\`\n${userId}\n\`\`\``,
      flags: MessageFlags.Ephemeral,
    });
...
export async function handleAvatarViewSourceButton(interaction: ButtonInteraction) {
  ...
  const app = loadApplication(appRow.id);
  if (!app) {
    await replyOrEdit(interaction, { content: "Application not found." }).catch(() => undefined);
    return;
  }
  ...
  const scan = getScan(app.id);
  if (!scan || scan.reason === "none") {
    // NSFW avatar scan is a heuristic; missing data should not block staff.
    // Heuristic overview: src/features/avatarScan.ts — finalPct blends nsfwjs + edge/skin heuristic.
    await replyOrEdit(interaction, { content: "Avatar scan not available." }).catch(
      () => undefined
    );
    return;
  }
  ...
  const modal = new ModalBuilder()
    .setCustomId(`v1:avatar:confirm18:code${code}`)
    .setTitle("Confirm 18+");
  const input = new TextInputBuilder()
  ...
  await interaction.showModal(modal).catch((err) => {
    logger.warn({ err, code }, "Failed to show avatar view source modal");
  });
}
...
export async function handleAvatarConfirmModal(interaction: ModalSubmitInteraction) {
  ...
  if (confirmation !== "I AM 18+") {
    await replyOrEdit(interaction, { content: "No." }).catch(() => undefined);
    return;
  }
  ...
  if (!cfg) {
    await replyOrEdit(interaction, { content: "Configuration missing." }).catch(() => undefined);
    return;
  }
  ...
  const user = await interaction.client.users.fetch(app.user_id).catch((err) => {
    logger.warn({ err, userId: app.user_id }, "[review] failed to fetch user for avatar confirm");
    return null;
  });
  const avatarUrl = user?.displayAvatarURL({ size: 256 }) ?? null;
  if (!avatarUrl) {
    await replyOrEdit(interaction, { content: "Avatar not available." }).catch(() => undefined);
    return;
  }
...
export async function ensureReviewMessage(
  ...
  `
      ...
      .get(appId) as
      | {
          action: string; // All action types: approve, reject, perm_reject, claim, kick, copy_uid, avatar_viewsrc, etc.
          moderator_id: string;
          reason: string | null;
          meta: string | null;
    ...
    let avatarScan: AvatarScanRow | null = null;
    try {
    ...
    if (message) {
      ...
      const code = shortCode(app.id);
      const roleId = guildCfg?.gatekeeper_role_id ?? "896070888762535969";
      const content = `<@&${roleId}> New application from <@${app.user_id}> • App #${code}`;
      ...
      logger.info({ roleId, guildId: app.guild_id, code }, "[review] gatekeeper ping sent");
    }
...
export async function handlePingInUnverified(interaction: ButtonInteraction) {
  ...
  try {
    ...
    logger.error(
      { err, userId, guildId: interaction.guildId },
    ...
    await replyOrEdit(interaction, {
      content: "Failed to post ping in unverified channel. Check logs.",
    }).catch(() => undefined);
  }
}
