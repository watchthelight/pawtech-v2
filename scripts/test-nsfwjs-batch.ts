 ...
 * Usage: npx tsx scripts/test-nsfwjs-batch.ts <directory>
 */

import fs from "node:fs";
import path from "node:path";

// Use dynamic import for nsfwjs due to module issues
...
for (let i = 0; i < files.length; i++) {
  ...
  try {
    ...
    const sexy = predictions.find((p: any) => p.className.toLowerCase() === "sexy")?.probability ?? 0;
